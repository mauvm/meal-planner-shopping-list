...
@singleton()
export default class ListItemStore implements AutoLoadableStore {
  ...
  async init() {
    ...
    this.eventStore.catchUpStream('list-items', (event: Event) => {
  ...
  async waitForEventFromStore(
    ...
    return new Promise((resolve, reject) => {
      ...
      let handler: EventEmitter.IHandler<Event, ListItemStore>
  ...
  async persistEvent(event: Event): Promise<void> {
    const eventId = await this.eventStore.persistEvent('list-items', event)
